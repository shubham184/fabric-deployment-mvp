...
variable "tags" {
  ...
  default = {
    environment = "test"
    project     = "fabric-deployment-testing"
  }
}
...
variable "debug_mode" {
  ...
  default     = false # Default to false for stable testing
}
...
variable "enable_test_features" {
  type        = bool
  description = "Enable test-specific features and validation"
  default     = true
}

variable "test_data_isolation" {
  type        = bool
  description = "Enable test data isolation to prevent interference"
  default     = true
}

# Advanced Configuration Variables (Optimized for Testing)
variable "pipeline_schedule" {
  ...
  description = "Pipeline scheduling configuration for testing"
  default = {
    enabled    = false    # Disabled by default for testing
    frequency  = "Hour"
    interval   = 12       # Every 12 hours for testing
...
variable "lakehouse_settings" {
  ...
  description = "Test lakehouse configuration settings"
  default = {
    enable_schemas      = true
    default_schema_name = "test_schema"
    enable_delta_lake   = true
    enable_auto_tune    = false  # Disabled for predictable test behavior
  }
}
...
variable "workspace_settings" {
  ...
  description = "Test workspace configuration settings"
  default = {
    enable_git_integration = true    # Enable for testing
    git_repository_url     = ""      # To be provided if git integration is enabled
    git_branch            = "test"   # Test branch
...
variable "resource_name_overrides" {
  ...
  description = "Optional resource name overrides for test environment"
  default = {
    workspace_name = ""
...
# Test Environment Validation Variables
...
variable "max_deployment_time" {
  type        = number
  description = "Maximum deployment time in minutes for test environment"
  default     = 45
...
variable "automated_testing" {
  type        = bool
  description = "Enable automated testing suite"
  default     = true
}

# Test Environment Cleanup Variables
